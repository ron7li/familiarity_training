...
def extract_final_block_features(image_encoder, images):
    ...
    the output shape is [seq_len, batch, hidden_dim] (not through the last mapping layer).
    """
    container = {}
...
def train_one_epoch(image_encoder, decoder, train_loader, optimizer, device):
    """
    During training:
      - Get the clean image from train_loader;
      - Add noise to the image at the beginning of training (here we use Gaussian noise, the noise amplitude is noise_strength);
      - Send the noisy image to the encoder to extract the transformer block output, and then reconstruct the image with the decoder;
      - Calculate the MSE loss between the reconstructed image and the original image, and update the encoder and decoder parameters.
    """
    image_encoder.train()
    decoder.train()
    total_loss = 0.0
...
def main(train_epoch=500, decoder_pretrained=False, decoder_pretrained_path=None, vit_type="ViT-B", img_patch_size=32, batch_size=8, lr=1e-4, device="cuda"):
    set_seed()
    

    # Load CLIP model (take the visual encoder part)
    model, preprocess = clip.load(vit_type + "/" + str(img_patch_size), device=device)
    model.float()
    image_encoder = model.visual
    
    # Build transformer decoder, set parameters reference to the MAE decoder structure
    ...
    # Construct the dataset (here only load the clean images, no noise processing)
    image_paths = []
    labels = []
    ...
    train(image_encoder, decoder, train_loader, device, num_epochs=train_epoch, lr=lr, log_dir="wo_lora")
...
if __name__ == "__main__":
    ...
    main(train_epoch=args.train_epoch, 
