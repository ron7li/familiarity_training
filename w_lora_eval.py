...
def load_results_from_json(json_path):
    ...
    acc_list = data["wo_lora"]["acc"]
    prob_list = data["wo_lora"]["prob"]
    cos_list = data["wo_lora"]["cos"]
    pcc_list = data["wo_lora"]["pcc"]

    return acc_list, prob_list, cos_list, pcc_list
...
def validate(image_encoder, 
    ...
    if using_noise:
        if num_noise_pattern == 5:
            epoch0_json_path = f"wo_lora/noise_ratio_{noise_ratio}/eval_epoch_{eval_epoch}/output_data/lora_results.json"
        else:
            epoch0_json_path = f"wo_lora/noise_ratio_{noise_ratio}/eval_epoch_{eval_epoch}_num_noise_pattern_{num_noise_pattern}/output_data/lora_results.json"
    else:
        epoch0_json_path = f"wo_lora/clean/eval_epoch_{eval_epoch}/output_data/lora_results.json"
    # load results from epoch 0
    layer_acc_outputs_, layer_prob_outputs_, layer_cos_outputs_, layer_pcc_outputs_ = load_results_from_json(epoch0_json_path)
    
    layer_acc_outputs.append(layer_acc_outputs_[0])
    layer_prob_outputs.append(layer_prob_outputs_[0])
    layer_cos_outputs.append(layer_cos_outputs_[0])
    layer_pcc_outputs.append(layer_pcc_outputs_[0])
    cpts.append(0)
    ...
    for i in range(1, 6):
        ...
        # load model weights
        ckpt_path = os.path.join(os.path.join('/'.join(log_dir.split('/')[:-2]), 'ckpt'), f'epoch_{epoch}.pt')
        ...
        layer_cos, layer_pcc = validate_layer_features(
            image_encoder=image_encoder,
            val_loader=val_loader,
            device=device,
            visualize=False,
...
def show_reconstruction(image_encoder, decoder, val_loader, device, log_dir, n_samples=4):
    """
    Show the comparison results of original image pairs and reconstructed images
    """
    image_encoder.eval()
    decoder.eval()
...
def main(using_noise=True, 
    ...
    for lrk in lora_ranks:
        ...
        if using_noise:
            if num_noise_pattern == 5:
                log_dir = os.path.join(log_dir, "lora_in_proj_initial_not_train", f"lora_r{lrk}", f"noise_ratio_{noise_ratio}", f"eval_epoch_{eval_epoch}")
            else:  
                log_dir = os.path.join(log_dir, "lora_in_proj_initial_not_train", f"lora_r{lrk}", f"noise_ratio_{noise_ratio}", f"eval_epoch_{eval_epoch}_num_noise_pattern_{num_noise_pattern}")
        else:
            log_dir = os.path.join(log_dir, "lora_in_proj_initial_not_train", f"lora_r{lrk}", "clean", f"eval_epoch_{eval_epoch}")
        ...
        layer_acc_outputs, layer_prob_outputs, layer_cos_outputs, layer_pcc_outputs = validate(
            ...
            num_noise_pattern=num_noise_pattern,
            if_attention_map=if_attention_map
        )
        
        output_dic[lrk] = (layer_acc_outputs, layer_prob_outputs, layer_cos_outputs, layer_pcc_outputs)
